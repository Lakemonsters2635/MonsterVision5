...
def printDeviceInfo(devInfo: dai.DeviceInfo):
    ...
    try:
        ...
        # If probing fails
        print("   >>> probing failed for MXID:", mxId)
...
def profile():
    with contextlib.ExitStack() as stack:
        ...
        for deviceInfo in deviceInfos:
            ...
            cam1 = camPipe.CameraPipeline(
                cm.mvConfig.getCamera(mxId)["name"],
                deviceInfo,
                useDepth=True,
                nnFile="/boot/nn.json",
            ...
            sdn = (
                cam1.setupSDN()
            )  # In the camera pipeline call another method to set up the spacial detection network for it given the NN config file and return a spacial detection node

            # Now build the pipeline

            cam1.buildPipeline(
                sdn, cm.mvConfig.getCamera(mxId)["invert"]
            )  # Build the pipeline with the above returned spacial detection node and the info of if the camera is inverted or not
            ...
            detector = Detections(
                cam1.bbfraction, cam1.LABELS
            )  # Create a detector object for the camera given some device and NN info
            tagDetector = AprilTag(
                cm.mvConfig.tagFamily,
                cm.mvConfig.tagSize,
                cam1.cameraIntrinsics,
                robotpy_apriltag.AprilTagField.k2024Crescendo,
            )  # Create an AprilTag detection object and supply the tag family, size, some camera intrinsics, and a special variable depending on the FRC season

            # Add the camera to the list of cameras, along with the detectors, etc.

            oakCameras.append(
                (cam1, mxId, detector, tagDetector)
            )  # add a tuple of the camera pipeline, cameraID, object detector, and AprilTag detector to the list of cameras
            ...
            for cam, mxId, detector, tagDetector in oakCameras:
                ...
                if cam.processNextFrame():
                    ...
                    if (
                        ...
                        and cam.detections is not None
                        and len(cam.detections) != 0
                    ):
                        objects = detector.processDetections(
                            cam.detections, cam.frame, cam.depthFrameColor
                        )
                    ...
                    if tagDetector is not None and cam.frame is not None:
                        ...
                        cv2.putText(
                            cam.frame,
                            "fps: {:.2f}".format(cam.fps),
                            (2, cam.frame.shape[0] - 4),
                            cv2.FONT_HERSHEY_TRIPLEX,
                            1.0,
                            (255, 255, 255),
                        )

                    res = frc.sd.putString(
                        "ObjectTracker-fps", "fps : {:.2f}".format(cam.fps)
                    )
