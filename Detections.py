...
class Detections:
    ...
    def ProcessOak1Detections(self, detections, frame):
        ...
        for detection in detections:
            ...
            detectionBB = dai.Rect(
                dai.Point2f(detection.xmin, detection.ymin),
                dai.Point2f(detection.xmax, detection.ymax),
            )
            roiDenorm = detectionBB.denormalize(
                cm.nnConfig.inputSize[0], cm.nnConfig.inputSize[1]
            )

            # Now map the denormalized bounding box to the full frame coordinates
            roiMapped = mapDetectionCoordinatesToFrame(
                roiDenorm, cm.nnConfig.inputSize, frame.shape
            )
            ...
            objects.append(
                {
                    "objectLabel": self.LABELS[detection.label],
                    "x": cX,
                    "y": cY,
                    "z": R,
    ...
    def processDetections(self, detections, frame, depthFrameColor):
        ...
        objects = []
        s_detections = sorted(
            detections, key=lambda det: det.label * 100000 + det.spatialCoordinates.z
        )
        # print(s_detections)
        ...
        for detection in s_detections:
            ...
            avg_pt1, avg_pt2 = _average_depth_coord(
                [xmin, ymin], [xmax, ymax], self.bbfraction
            )
            ...
            cv2.rectangle(
                frame, (x1, y1), (x2, y2), (0, 0, 255), cv2.FONT_HERSHEY_SIMPLEX
            )
