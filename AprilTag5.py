...
class AprilTag:
    ...
    def detect(self, image, depthFrame):
        ...
        for detection in detections:
            ...
            cv2.putText(
                image, tagID, (lblX, lblY - 75), cv2.FONT_HERSHEY_TRIPLEX, 0.5, color
            )
            cv2.putText(
                image,
                f" X: {round(pose.X()*METERS_TO_INCHES, 1)} {units}",
                (lblX, lblY - 60),
                cv2.FONT_HERSHEY_TRIPLEX,
                0.5,
                color,
            ...
            cv2.putText(
                image,
                f" Y: {round(pose.Y()*METERS_TO_INCHES, 1)} {units}",
                (lblX, lblY - 45),
                cv2.FONT_HERSHEY_TRIPLEX,
                0.5,
                color,
            ...
            cv2.putText(
                image,
                f" Z: {round(pose.Z()*METERS_TO_INCHES, 1)} {units}",
                (lblX, lblY - 30),
                cv2.FONT_HERSHEY_TRIPLEX,
                0.5,
                color,
            ...
            cv2.putText(
                image,
                f"XA: {round(rot.x_degrees, 1)} deg",
                (lblX, lblY - 15),
                cv2.FONT_HERSHEY_TRIPLEX,
                0.5,
                color,
            ...
            cv2.putText(
                image,
                f"YA: {round(rot.y_degrees, 1)} deg",
                (lblX, lblY + 0),
                cv2.FONT_HERSHEY_TRIPLEX,
                0.5,
                color,
            ...
            cv2.putText(
                image,
                f"ZA: {round(rot.z_degrees, 1)} deg",
                (lblX, lblY + 15),
                cv2.FONT_HERSHEY_TRIPLEX,
                0.5,
                color,
            ...
            objects.append(
                {
                    "objectLabel": tagID + ": " + str(detection.getId()),
                    "x": round(pose.X() * METERS_TO_INCHES, 1),
                    "y": round(pose.Y() * METERS_TO_INCHES, 1),
                    "z": round(pose.Z() * METERS_TO_INCHES, 1),
                    "confidence": 1.0,
                    "xa": round(rot.x_degrees, 1),
                    "ya": round(rot.y_degrees, 1),
                    "za": round(rot.z_degrees, 1),
                }
            )
